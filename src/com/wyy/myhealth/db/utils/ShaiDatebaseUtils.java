...
public class ShaiDatebaseUtils implements DatebaseInterface{
	...
	public Object delete(long id) {
		...
		int del=-1;
		 try{
	    	 ...
	    	 del=WyyDatebase.wyyDatabase.delete(ShaiData.TABLE_NAME, where, whereArgs);
	    }catch (Exception e){
	      e.printStackTrace();
	    }
		return del;
	...
	public long insert(String json, int postion) {
    	...
    	try {
	    	...
	    	//调用方法插入数据
	    	id = WyyDatebase.wyyDatabase.insert(ShaiData.TABLE_NAME, null, values);	
    	} catch (Exception e) {
			e.printStackTrace();
		}
		return id;
	...
	public Object queryData() {
    	...
    	try {
			//获得数据库对象,如过数据库不存在则创建
			//查询表中数据,获取游标
			cursor = WyyDatebase.wyyDatabase.query(ShaiData.TABLE_NAME,null,null,null,null,null,"_ID desc");
			//获取name列的索引
	...
	public long update(String json, int postion, int id) {
    	...
    	try {
	    	...
	    	//调用方法插入数据
	    	updatepostion=WyyDatebase.wyyDatabase.update(ShaiData.TABLE_NAME, values, where, whereValue);
    	} catch (Exception e) {
			e.printStackTrace();
		}
